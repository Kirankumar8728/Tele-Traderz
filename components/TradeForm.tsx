...
const TradeForm: React.FC<TradeFormProps> = ({ 
  ...
  return (
    <div className={`z-30 ${compact ? 'space-y-1.5 p-0' : 'space-y-5 p-2.5 bg-[#0c0f17] rounded-3xl border border-white/5 shadow-2xl'}`}>
      {/* Trading Error Display (Moved Above Form) */}
      {error && (
        <div 
          id="table-error-msg" 
          className="bg-rose-500/10 text-rose-400 border border-rose-500/15 p-2 rounded-xl text-[10px] text-center font-bold animate-in fade-in slide-in-from-top-1 duration-300"
        >
          {error}
        </div>
      )}
      ...
      <div className={`flex ${compact ? 'gap-1' : 'gap-1.5'} overflow-x-auto no-scrollbar pb-0.5`}>
        {[
          ...
          <button
            ...
            className={`flex items-center ${compact ? 'gap-1 px-2.5 py-1 text-[9px]' : 'gap-1.5 px-4 py-2.5 text-[10.5px]'} rounded-full font-black uppercase whitespace-nowrap transition-all border ${
              tradeType === type.id
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {/* Stake / Payout Input */}
        <div className={`bg-white/[0.02] hover:bg-white/[0.04] ${compact ? 'rounded-xl p-1.5' : 'rounded-2xl p-3'} border border-white/5 flex flex-col justify-between transition-colors`}>
          <div className="flex justify-between items-center mb-0.5">
            ...
            <button
              ...
              className={`${compact ? 'text-[7px] px-1.5 py-0.5' : 'text-[8.5px] px-2.5 py-0.5'} font-black text-red-500 hover:text-red-400 uppercase tracking-wide bg-red-500/10 hover:bg-red-500/20 rounded-full transition-all`}
            >
          ...
          <input 
            ...
            className={`w-full bg-transparent ${compact ? 'text-xs' : 'text-sm'} font-mono font-extrabold text-white outline-none mt-0.5`} 
          />
        </div>

        {/* Duration Input */}
        <div className={`bg-white/[0.02] hover:bg-white/[0.04] ${compact ? 'rounded-xl p-1.5' : 'rounded-2xl p-3'} border border-white/5 flex justify-between items-center transition-colors`}>
          <div className="flex-1">
            <span className={`block ${compact ? 'text-[7.5px]' : 'text-[8.5px]'} font-black text-gray-400 uppercase mb-0.5 tracking-widest`}>DURATION</span>
            <input 
              type="number" 
              min="1"
              value={duration} 
          ...
          <div className="relative flex items-center gap-1">
            <select 
              ...
              className={`bg-transparent ${compact ? 'text-[9px]' : 'text-[10px]'} font-black text-red-500 uppercase outline-none cursor-pointer pr-3.5 appearance-none text-right select-none`}
            >
            ...
            <ChevronDown className="w-3 h-3 text-red-500 absolute right-0 pointer-events-none stroke-[3px]" />
          </div>
        </div>

        {/* Barrier Input (Conditional) */}
        {['HIGHER', 'LOWER', 'TOUCH', 'NOTOUCH', 'ONETOUCH'].includes(tradeType) && (
          <div className={`col-span-2 bg-white/[0.02] hover:bg-white/[0.04] ${compact ? 'rounded-xl p-1.5' : 'rounded-2xl p-3'} border border-white/5 flex flex-col justify-between transition-colors`}>
            <div className="flex justify-between items-center mb-0.5">
              ...
              <span className="text-[8.5px] font-mono font-bold text-gray-400 bg-white/5 px-1.5 py-0.5 rounded border border-white/5">
            ...
            <div className="relative flex items-center">
              ...
              {barrier.match(/^[+-]/) && lastPrice > 0 && !isNaN(parseFloat(barrier)) && (
                <div className="text-[8.5px] font-mono font-extrabold text-gray-400 ml-2 whitespace-nowrap bg-red-600/10 border border-red-500/10 px-1.5 py-0.5 rounded">
      ...
      <div className={`flex ${compact ? 'gap-1.5 mb-1' : 'gap-4 mb-2'}`}>
        {/* RISE (Call) Column */}
        <div className={`flex-1 flex flex-col ${compact ? 'gap-0.5' : 'gap-2.5'}`}>
          {/* Estimate Details */}
          <div className="grid grid-cols-2 gap-0.5 px-0.5 text-[8.5px] font-bold text-gray-400">
            <div className="flex flex-col text-left">
              <span className={`${compact ? 'text-[6.5px]' : 'text-[7.5px]'} text-gray-500 uppercase tracking-widest font-black`}>Est. Payout</span>
              <span className={`${compact ? 'text-[9.5px]' : 'text-[11px]'} font-mono font-extrabold text-emerald-400 mt-0.5`}>
                {basis === 'stake' 
            ...
            <div className="flex flex-col text-right">
              <span className={`${compact ? 'text-[6.5px]' : 'text-[7.5px]'} text-gray-500 uppercase tracking-widest font-black`}>Net Profit</span>
              <span className={`${compact ? 'text-[9.5px]' : 'text-[11px]'} font-mono font-extrabold text-emerald-400 mt-0.5`}>
          ...
          <button 
            ...
            className={`${compact ? 'h-10 rounded-xl' : 'h-14 rounded-2xl'} bg-emerald-500 hover:bg-emerald-400 active:scale-[0.98] transition-all flex items-center justify-center px-2 shadow-lg shadow-emerald-500/10 disabled:opacity-50 border border-white/5 relative overflow-hidden group green-glow cursor-pointer`}
          >
            <div className="flex items-center gap-1.5">
              {buyingTypes.has(upData.type) ? (
                <Loader2 className={`${compact ? 'w-3.5 h-3.5' : 'w-5 h-5'} text-white animate-spin`} />
              ) : (
                <TrendingUp className={`${compact ? 'w-3.5 h-3.5' : 'w-5 h-5'} text-white stroke-[3px]`} />
              )}
        ...
        <div className={`flex-1 flex flex-col ${compact ? 'gap-0.5' : 'gap-2.5'}`}>
          ...
          <div className="grid grid-cols-2 gap-0.5 px-0.5 text-[8.5px] font-bold text-gray-400">
            <div className="flex flex-col text-left">
              <span className={`${compact ? 'text-[6.5px]' : 'text-[7.5px]'} text-gray-500 uppercase tracking-widest font-black`}>Est. Payout</span>
              <span className={`${compact ? 'text-[9.5px]' : 'text-[11px]'} font-mono font-extrabold text-rose-400 mt-0.5`}>
                {basis === 'stake' 
          ...
          <button 
            ...
            className={`${compact ? 'h-10 rounded-xl' : 'h-14 rounded-2xl'} bg-rose-600 hover:bg-rose-500 active:scale-[0.98] transition-all flex items-center justify-center px-2 shadow-lg shadow-rose-600/10 disabled:opacity-50 border border-white/5 relative overflow-hidden group red-glow cursor-pointer`}
          >
            <div className="flex items-center gap-1.5">
              {buyingTypes.has(downData.type) ? (
                <Loader2 className={`${compact ? 'w-3.5 h-3.5' : 'w-5 h-5'} text-white animate-spin`} />
              ) : (
                <TrendingDown className={`${compact ? 'w-3.5 h-3.5' : 'w-5 h-5'} text-white stroke-[3px]`} />
              )}
              <span className={`${compact ? 'text-xs' : 'text-sm'} font-extrabold text-white tracking-widest uppercase truncate`}>
                {downData.label}
              </span>
