...
export const generateCodeVerifier = () => {
  const array = getRandomValues(new Uint8Array(64));
...
export const generateState = () => {
  const array = getRandomValues(new Uint8Array(16));
  return Array.from(array)
...
function sha256(ascii: string): Uint8Array {
  ...
  for (let i = 0; i < words.length; i += 16) {
    ...
    for (let j = 0; j < 64; j++) {
      ...
      const t1 = (hash[7] + sigma1 + ch + k[j] + (w[j] || 0)) | 0;
  ...
  for (let i = 0; i < 8; i++) {
    uint8[i * 4] = (hash[i] >>> 24) & 0xff;
    uint8[i * 4 + 1] = (hash[i] >>> 16) & 0xff;
    uint8[i * 4 + 2] = (hash[i] >>> 8) & 0xff;
    uint8[i * 4 + 3] = hash[i] & 0xff;
  }
  return uint8;
}
